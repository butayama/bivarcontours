...
def post_order(node):
    ...
    if isinstance(node, ast.Name):
        ...
        if isinstance(node.op, ast.Mult):
            r_dim = left_dim + right_dim  # assuming that the dimensions are additive
        elif isinstance(node.op, ast.Add) or isinstance(node.op, ast.Sub):  # Add or Sub
            r_dim = left_dim  # the dimension should be the same for both left and right and stays as is
        elif isinstance(node.op, ast.Div):  # Div
            r_dim = left_dim if left_dim != right_dim else ''  # if dimensions are equal it results in a dimensionless entity
        else:
